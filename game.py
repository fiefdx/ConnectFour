...
class Game(object):
    def __init__(self, think_games = 100, mode = "menu", task_queue = None, result_queue = None):
        ...
        self.minimax_levels_map = {
            10: 3, 20: 4, 30: 5, 50: 6, 100: 7, 200: 8, 500: 9,
            1000: 3, 2000: 4, 5000: 5, 10000: 6, 20000: 7, 25000: 8, 50000: 9,
        }
    ...
    def choose_immediate_move(self):
        if self.discs_counter < 2:
            return 3
        best_x = self.check_win_move()
        if best_x == -1:
    ...
    def iter_best_move_tasks(self):
        xs = self.available_place_xs()
        for x in xs:
            g = GamePicklable()
            g.copy_from(self)
            g.turn_place_disc(x)
            yield (self.think_mode, g, x, self.think_games)
